...
function TextMode({ todo, edited, selected, i }) {
  ...
  let bg = React.useRef()
  useEffect(() => {
    if (todo.selected) bg.current.classList.add('BoxSelected')
    else bg.current.classList.remove('BoxSelected')
  }, [{ todo }])
...
function AddMode({ added, selected }) {
  ...
  return (
    <div className="TaskBox AddBox">
      ...
      >
        <div
          className="AddBoxContents"
          onClick={e => {
            e.target.className = 'TextBoxContents'
